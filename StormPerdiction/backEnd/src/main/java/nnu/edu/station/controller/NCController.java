...
public class NCController {
    ...
    @GetMapping({"/field/velocity/point"})
    public ResponseEntity<FileSystemResource> getVellcityByPoint(@RequestParam String name) throws IOException, JSONException {
        String filePath = this.WaterLevel + "/" + name + ".json";
        File file = new File(filePath);
        if (file.exists()) {
            HttpHeaders headers = new HttpHeaders();
            headers.add("Content-Disposition", "attachment; ftiilename=" + name);
            headers.add("Content-Type", "application/json");
            return ResponseEntity.ok().headers(headers).contentLength(file.length()).contentType(MediaType.APPLICATION_JSON).body(new FileSystemResource(file));
        } else {
            return ResponseEntity.notFound().build();
        }
    }
